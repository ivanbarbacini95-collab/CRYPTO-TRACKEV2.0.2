...
async function fetchHistory() {
    ...
    // settimanale e mensile semplificato con gli stessi dati (da integrare con dati reali)
...
function updateBar(bar, line, val, open, low, high, gradUp, gradDown) {
    const percent = (val - low) / (high - low) * 100;
    line.style.left = percent + "%";

    const centerPercent = (open - low) / (high - low) * 100;
    if (val >= open) {
        const width = percent - centerPercent;
        bar.style.left = centerPercent + "%";
        bar.style.width = width + "%";
        bar.style.background = gradUp;
    } else {
        const width = centerPercent - percent;
