...
async function fetchHistoryToday(){
  ...
  const minutesSinceMidnight = Math.floor((Date.now() - startOfDay)/60000);

  const d = await fetchJSON(`https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=1m&limit=${minutesSinceMidnight}`);
        ...
        x:{
          ticks:{
            ...
            callback: function(val,index){
              ...
              return label.endsWith(":00") ? label : ''; // solo ore principali
            },
            autoSkip:false,
            maxRotation:0,
            minRotation:0
