...
function initChart24h(){
  ...
  chart = new Chart(ctx,{
    ...
    options:{ responsive:true, maintainAspectRatio:false, animation:false, plugins:{ legend:{display:false} }, scales:{ x:{ ticks:{color:"#9ca3af", autoSkip:false, maxTicksLimit:25} }, y:{ ticks:{color:"#9ca3af"} } } }
  });
...
function updateChartRealtime(price){
  ...
  const now = new Date();
  chartLabels.push(`${now.getHours().toString().padStart(2,'0')}:${Math.floor(now.getMinutes()/10)*10}`);
  ...
  if(lastPoint){
    const pos = lastPoint.getCenterPoint();
