...
function colorNumber(el,curr,prev,dec=4){
  const s1=curr.toFixed(dec),s2=prev.toFixed(dec);
  let html="";
...
function updateValue(ref,newVal){
  if(Math.abs(newVal-ref.displayed)>0.00001){ const old=ref.displayed; ref.displayed=newVal; colorNumber(ref.el,ref.displayed,old,ref.decimals); }
  else { const old=ref.displayed; ref.displayed+=(newVal-ref.displayed)*0.3; if(Math.abs(ref.displayed-newVal)<1e-6) ref.displayed=newVal; colorNumber(ref.el,ref.displayed,old,ref.decimals);}
}
...
function updateBar(barEl,lineEl,displayed,open,low,high){
  ...
  if(displayed>=open){ barEl.style.left="50%"; barEl.style.width=(linePerc-50)+"%"; barEl.style.background="linear-gradient(to right,#22c55e,#10b981)"; }
  else{ barEl.style.left=linePerc+"%"; barEl.style.width=(50-linePerc)+"%"; barEl.style.background="linear-gradient(to left,#ef4444,#f87171)"; }
}
...
function animate(){
  ...
  updateValue(values.stake,stakeInj); $("stakeUsd").textContent=`≈ $${(
