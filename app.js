...
const mockIndices = [
  {symbol:"S&P500", price:4100, up:true},
  {symbol:"NASDAQ", price:12500, up:false},
  {symbol:"DOWJ", price:33000, up:true},
  {symbol:"FTSE", price:7800, up:false},
  {symbol:"DAX", price:15800, up:true}
];
...
function renderTicker() {
  ...
  mockIndices.forEach(i => {
    ...
    div.innerHTML = `
      ...
      <span class="status-dot" style="background:${i.up?'#22c55e':'#ef4444'}"></span>
    `;
    div.onclick = ()=>openMiniChart(i.symbol);
...
/* MINI CHART */
function openMiniChart(symbol){
  const modal = $("chartModal");
  modal.classList.remove("hidden");
  const prices = Array.from({length:24},()=>Math.random()*100 + 100);
  if(miniChart) miniChart.destroy();
  miniChart = new Chart($("miniChart"),{
    type:"line",
    data:{labels:Array(24).fill(""), datasets:[{data:prices,borderColor:"#3b82f6",pointRadius:0}]},
