...
/* ================= PATCH INTEGRATIVA v2.0.2+ (append-only) ================= */
(function PATCH_20260204(){
  const PATCH_VER = "patch_20260204_v1";

  const $$ = (sel, root=document) => Array.from(root.querySelectorAll(sel));
  const byId = (id) => document.getElementById(id);

  function once(key){
    const k = `__${PATCH_VER}_${key}`;
  ...
  function toast(msg){
    try{
      // reuse your toast style if exists, else quick alert-like toast
      ...
      t.style.background = (document.body.dataset.theme==="light") ? "rgba(15,23,42,.88)" : "rgba(249,250,251,.92)";
      t.style.color = (document.body.dataset.theme==="light") ? "rgba(249,250,251,.96)" : "rgba(15,23,42,.96)";
      t.style.boxShadow = "0 18px 70px rgba(0,0,0,.45)";
      t.textContent = msg || "Done";
      document.body.appendChild(t);
      setTimeout(()=>{ t.style.opacity = "0"; t.style.transition="opacity 220ms ease"; }, 1000);
  ...
  function ensureCopyBtn(){
    const host = byId("addrCopyMount") || byId("addressDisplay")?.parentElement || byId("addressDisplay");
    const display = byId("addressDisplay");
    ...
    btn.innerHTML = `<span class="ico">📋</span><span class="txt">Copy</span>`;
    ...
    btn.addEventListener("click", async (e) => {
      ...
      e.stopPropagation();
      try{
        const a = (typeof address !== "undefined") ? (address || "") : "";
        if (!a) return;
        ...
        if (ok){
          ...
          btn.querySelector(".txt").textContent = "Copied ✓";
          setTimeout(()=>{ btn.querySelector(".txt").textContent = "Copy"; }, 900);
        } else {
          toast("Copy failed");
        }
      }catch{
    ...
    // place it next to addressDisplay line
    host.appendChild(btn);
  }

  // Wrap setAddressDisplay to keep the copy button consistent
  if (typeof setAddressDisplay === "function" && once("wrapSetAddressDisplay")){
    const _orig = setAddressDisplay;
    setAddressDisplay = function(addr){
      _orig(addr);
      ensureCopyBtn();
    };
  }

  /* ---------- 2) VALIDATOR: pick main validator = biggest delegation ---------- */
  async function refreshMainValidatorFromLCD(){
    try{
      if (typeof hasInternet === "function" && !hasInternet()) return;
      if (typeof address === "undefined" || !address) return;

      const base = "https://lcd.injective.network";
      const r = await fetchJSON(`${base}/cosmos/staking/v1beta1/delegations/${address}`);
      const delgs = (r?.delegation_responses || []);
      if (!delgs.length){
        if (typeof loadValidatorInfo === "function") loadValidatorInfo("");
        return;
      }
      ...
      const opAddr = best?.delegation?.validator_address || "";
      if (opAddr && typeof loadValidatorInfo === "function") loadValidatorInfo(opAddr);
    }catch{}
  }

  // Wrap loadAccount to enforce validator-in-card (no random placement)
  if (typeof loadAccount === "function" && once("wrapLoadAccountValidator")){
    const _origLoadAccount = loadAccount;
    loadAccount = async function(isRefresh=false){
      const out = await _origLoadAccount(isRefresh);
      refreshMainValidatorFromLCD();
      return out;
    };
  }

  /* ---------- 3) STAKE CHART: point ONLY on increase (baseline ok) ---------- */
  if (typeof maybeAddStakePoint === "function" && once("wrapMaybeAddStakePoint")){
    const _origMaybe = maybeAddStakePoint;
    maybeAddStakePoint = function(currentStake){
      try{
        const s = safe(currentStake);
        const rounded = Number(s.toFixed(6));

        // let original create baseline if needed
        if (typeof stakeBaselineCaptured !== "undefined" && !stakeBaselineCaptured) {
          return _origMaybe(currentStake);
        }

        // if no last, set and exit
        ...
        if (rounded <= lastStakeRecordedRounded){
          lastStakeRecordedRounded = rounded; // keep tracking without adding points
          return;
        }
  ...
  // Extra guard: when switching address, clear in-memory series if load fails
  if (typeof commitAddress === "function" && once("wrapCommitAddressClearSeries")){
    const _origCommit = commitAddress;
    commitAddress = async function(newAddr){
      try{
        // hard clear runtime arrays to avoid visual mixing if load fails
        ...
        if (typeof wdLabelsAll !== "undefined") wdLabelsAll = [];
        if (typeof wdValuesAll !== "undefined") wdValuesAll = [];
        if (typeof wdTimesAll !== "undefined") wdTimesAll = [];
      } catch {}
      const out = await _origCommit(newAddr);
      refreshMainValidatorFromLCD();
      return out;
    };
  }

  /* ---------- 4) Global blinking last-dot plugin for ALL line charts (skip NW which already has one) ---------- */
  function registerLastDotPlugin(){
    try{
      if (!window.Chart) return;
      const already = Chart?.registry?.plugins?.get && Chart.registry.plugins.get("lastDotPulsePatch");
      if (already) return;

      const plugin = {
        id: "lastDotPulsePatch",
        afterDatasetsDraw(ch){
          try{
            // skip if chart already has NW last-dot plugin
            const localPlugins = ch?.config?.plugins || [];
            if (localPlugins.some(p => p && p.id === "nwLastDotPlugin")) return;
            ...
            ctx.shadowBlur = 10;
            ctx.beginPath();
            ctx.arc(el.x, el.y, 6.2, 0, Math.PI * 2);
  ...
  /* ---------- 5) Chart scale toggle (LIN/LOG) under charts ---------- */
  function getScaleKey(key){ return `inj_scale_${key}`; }

  function canUseLog(chartObj){
    try{
      const data = chartObj?.data?.datasets?.[0]?.data || [];
      if (!data.length) return false;
      // log requires positive values
      return data.every(v => Number.isFinite(+v) && +v > 0);
    }catch{ return false; }
  }

  function setChartScale(chartObj, scaleType){
    try{
      if (!chartObj) return;
      if (scaleType === "logarithmic" && !canUseLog(chartObj)){
        toast("Log scale needs values > 0");
        scaleType = "linear";
      }
      chartObj.options = chartObj.options || {};
      chartObj.options.scales = chartObj.options.scales || {};
      chartObj.options.scales.y = chartObj.options.scales.y || {};
      chartObj.options.scales.y.type = (scaleType === "logarithmic") ? "logarithmic" : "linear";
      chartObj.update("none");
    }catch{}
  }

  function ensureScaleControls(canvasId, key, getChartFn){
    const canvas = byId(canvasId);
    if (!canvas) return;
    const card = canvas.closest(".card") || canvas.parentElement;
    if (!card) return;

    if (card.querySelector(`.patch-scale-controls[data-key="${key}"]`)) return;

    const row = document.createElement("div");
    row.className = "patch-under-controls patch-scale-controls";
    row.dataset.key = key;

    const left = document.createElement("div");
    left.className = "patch-pill-row";
    left.innerHTML = `
      <button class="patch-pill" type="button" data-scale="linear">LIN</button>
      <button class="patch-pill" type="button" data-scale="logarithmic">LOG</button>
    `;

    const right = document.createElement("div");
    right.style.opacity = ".78";
    right.style.fontWeight = "900";
    right.style.fontSize = ".78rem";
    right.textContent = "";
    ...
    const btns = $$(".patch-pill", left);
    const saved = localStorage.getItem(getScaleKey(key)) || "linear";
    btns.forEach(b => b.classList.toggle("active", b.dataset.scale === saved));

    // apply immediately if chart exists
    const ch0 = getChartFn?.();
    if (ch0) setChartScale(ch0, saved);

    left.addEventListener("click", (e) => {
      const b = e.target?.closest(".patch-pill");
      if (!b) return;
      const scale = b.dataset.scale || "linear";
      localStorage.setItem(getScaleKey(key), scale);
      btns.forEach(x => x.classList.toggle("active", x === b));
      const ch = getChartFn?.();
      if (ch) setChartScale(ch, scale);
    }, { passive:true });
  }

  /* ---------- 6) PRICE CHART Timeframes + colored by sign ---------- */
  const PRICE_TF_KEY = "inj_price_tf_patch";
  const PRICE_MAX_POINTS_ALL = 2500;
  ...
  function fmtLabelForInterval(ms, interval){
    try{
      if (interval === "1d" || interval === "1w" || interval === "1M") return new Date(ms).toLocaleDateString();
      return (typeof fmtHHMM === "function") ? fmtHHMM(ms) : new Date(ms).toLocaleTimeString();
    }catch{
      return String(ms);
    }
  }

  async function fetchKlinesPaged(interval, startTime, endTime, cap){
    const out = [];
    let cursor = startTime;
    const end = endTime || Date.now();
    const max = cap || 1200;
    ...
    while (cursor < end && out.length < max){
      const url = `https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=${interval}&limit=1000&startTime=${cursor}&endTime=${end}`;
      const d = await fetchJSON(url);
      if (!Array.isArray(d) || !d.length) break;
      ...
      // step forward (binance returns open time)
      cursor = lastOpen + 1;
      if (d.length < 1000) break;
    }
  ...
  function applyPriceColorsFromSeries(){
    try{
      if (!chart) return;
      ...
      // reuse your function if exists
      if (typeof applyChartColorBySign === "function") {
        applyChartColorBySign(sign);
      } else {
        const ds = chart.data.datasets[0];
        if (sign === "up"){ ds.borderColor="#22c55e"; ds.backgroundColor="rgba(34,197,94,.20)"; }
        else if (sign === "down"){ ds.borderColor="#ef4444"; ds.backgroundColor="rgba(239,68,68,.18)"; }
        else { ds.borderColor="#3b82f6"; ds.backgroundColor="rgba(59,130,246,.14)"; }
        chart.update("none");
      }
    }catch{}
  }
  ...
  async function loadPriceTF(tfKey){
    try{
      const def = PRICE_TFS.find(x => x.k === tfKey) || PRICE_TFS.find(x => x.k === "1d");
      if (!def) return;

      if (typeof hasInternet === "function" && !hasInternet()) return;

      const end = Date.now();
      const start = def.winMs ? (end - def.winMs) : (end - 3650*24*60*60*1000);

      const cap = (def.k === "all") ? PRICE_MAX_POINTS_ALL : 1400;
      const kl = await fetchKlinesPaged(def.interval, start, end, cap);
      if (!kl.length) return;

      // reuse global arrays if present
      if (typeof chartLabels !== "undefined") chartLabels = kl.map(k => fmtLabelForInterval(safe(k[0]), def.interval));
      if (typeof chartData !== "undefined")   chartData   = kl.map(k => safe(k[4]));

      // make sure chart exists
      if (!chart && typeof initChartToday === "function") initChartToday();

      if (chart){
        chart.data.labels = (typeof chartLabels !== "undefined") ? chartLabels : chart.data.labels;
        chart.data.datasets[0].data = (typeof chartData !== "undefined") ? chartData : chart.data.datasets[0].data;
        chart.update("none");
        applyPriceColorsFromSeries();

        // apply saved scale
        const sc = localStorage.getItem(getScaleKey("price")) || "linear";
        setChartScale(chart, sc === "logarithmic" ? "logarithmic" : "linear");
      }

      localStorage.setItem(PRICE_TF_KEY, def.k);

      // mark as bootstrapped to stop the old ensure loop from overriding
      if (typeof chartBootstrappedToday !== "undefined") chartBootstrappedToday = true;
    }catch{}
  }

  function ensurePriceTfUI(){
    ...
    if (card.querySelector(".patch-price-tf")) return;

    const wrap = document.createElement("div");
    wrap.className = "patch-under-controls patch-price-tf";

    const left = document.createElement("div");
    left.className = "patch-pill-row";

    left.innerHTML = PRICE_TFS.map(x => `<button class="patch-pill" type="button" data-tf="${x.k}">${x.label}</button>`).join("");
    ...
    const savedTf = localStorage.getItem(PRICE_TF_KEY) || "1d";
    const tfBtns = $$(".patch-pill[data-tf]", left);
    tfBtns.forEach(b => b.classList.toggle("active", b.dataset.tf === savedTf));

    left.addEventListener("click", (e) => {
      const b = e.target?.closest(".patch-pill[data-tf]");
      if (!b) return;
      tfBtns.forEach(x => x.classList.toggle("active", x === b));
      loadPriceTF(b.dataset.tf);
    }, { passive:true });

    // scale buttons (reuses same key "price")
    const scBtns = $$(".patch-pill[data-scale]", right);
    const savedScale = localStorage.getItem(getScaleKey("price")) || "linear";
    scBtns.forEach(b => b.classList.toggle("active", b.dataset.scale === savedScale));
    right.addEventListener("click", (e) => {
      const b = e.target?.closest(".patch-pill[data-scale]");
      if (!b) return;
      const scale = b.dataset.scale || "linear";
      localStorage.setItem(getScaleKey("price"), scale);
      scBtns.forEach(x => x.classList.toggle("active", x === b));
      if (chart) setChartScale(chart, scale);
    }, { passive:true });
  ...
  /* ---------- 7) Reward filter options + estimates row ---------- */
  function patchRewardFilterOptions(){
    const sel = byId("rewardFilter");
    if (!sel || sel.dataset.patched === "1") return;
    sel.dataset.patched = "1";

    sel.innerHTML = `
      <option value="0">All</option>
      <option value="0.05">&lt; 0.05 (filter: >= 0.05 OFF)</option>
      <option value="0.05">≥ 0.05</option>
      <option value="0.1">≥ 0.10</option>
    `;
    // keep behavior: your handler reads value as wdMinFilter
  }

  function ensureRewardEstimatesUI(){
    const canvas = byId("rewardChart");
    if (!canvas) return;

    const card = canvas.closest(".card") || canvas.parentElement;
    if (!card) return;

    if (card.querySelector("#rewardEstimates")) return;

    const wrap = document.createElement("div");
    wrap.id = "rewardEstimates";
    wrap.className = "reward-estimates";
    wrap.innerHTML = `
      <div class="est"><div class="k">1H (est.)</div><div class="v" id="rewEst1h">—</div></div>
      <div class="est"><div class="k">1D (est.)</div><div class="v" id="rewEst1d">—</div></div>
      <div class="est"><div class="k">1W (est.)</div><div class="v" id="rewEst1w">—</div></div>
      <div class="est"><div class="k">1M (est.)</div><div class="v" id="rewEst1m">—</div></div>
      <div class="est"><div class="k">1Y (est.)</div><div class="v" id="rewEst1y">—</div></div>
    `;
  ...
  function updateRewardEstimates(){
    try{
      ...
      const aprPct = safe((typeof apr !== "undefined") ? apr : 0);

      if (!st || !aprPct) return;

      const year = st * (aprPct/100);
      const oneH = year / (365*24);
      const oneD = year / 365;
      const oneW = year / 52;
      const oneM = year / 12;

      const fmt = (x) => `${x.toFixed(6)} INJ${px ? `  (≈ $${(x*px).toFixed(2)})` : ""}`;

      const a = byId("rewEst1h"); if (a) a.textContent = fmt(oneH);
      const b = byId("rewEst1d"); if (b) b.textContent = fmt(oneD);
      const c = byId("rewEst1w"); if (c) c.textContent = fmt(oneW);
      const d = byId("rewEst1m"); if (d) d.textContent = fmt(oneM);
      const e = byId("rewEst1y"); if (e) e.textContent = fmt(year);
    }catch{}
  }

  /* ---------- 8) Gear targets for Stake/Reward bars (per address) ---------- */
  function keyTarget(kind, addr){
    return `inj_${kind}_target_${addr || "noaddr"}`;
  }

  function openTargetPopover(title, currentVal, onApply){
    const pop = document.createElement("div");
    pop.className = "patch-pop";
    pop.innerHTML = `
      <div class="patch-pop-card">
        <div class="patch-pop-title">${title}</div>
        <div class="patch-pop-row">
          <input id="patchTargetInput" inputmode="decimal" placeholder="Enter max value (e.g. 250)" value="${String(currentVal ?? "")}">
        </div>
        <div class="patch-pop-actions">
          <button type="button" id="patchCancel">Cancel</button>
          <button type="button" id="patchApply">Apply</button>
        </div>
        <div style="margin-top:10px;opacity:.75;font-weight:850;font-size:.8rem;">
          This affects only the progress bar scale (saved per wallet).
        </div>
      </div>
    `;
    document.body.appendChild(pop);

    const input = pop.querySelector("#patchTargetInput");
    setTimeout(()=>input?.focus(), 20);

    pop.querySelector("#patchCancel")?.addEventListener("click", ()=>pop.remove(), { passive:true });
    pop.addEventListener("click", (e)=>{ if (e.target === pop) pop.remove(); }, { passive:true });

    pop.querySelector("#patchApply")?.addEventListener("click", ()=>{
      const v = safe(input?.value);
      if (v <= 0) { toast("Invalid value"); return; }
      onApply(v);
      pop.remove();
    }, { passive:true });
  }
  ...
  function ensureGearButtons(){
    // Stake
    const stakeBar = byId("stakeBar");
    const stakeLine = byId("stakeLine");
    if (stakeBar && !byId("stakeGearBtn")){
      const host = stakeBar.parentElement?.parentElement || stakeBar.parentElement;
      if (host){
        const btn = document.createElement("button");
        btn.id = "stakeGearBtn";
        btn.type = "button";
        ...
        btn.addEventListener("click", ()=>{
          const a = (typeof address !== "undefined") ? address : "";
          const cur = safe(localStorage.getItem(keyTarget("stake", a))) || safe((typeof STAKE_TARGET_MAX !== "undefined") ? STAKE_TARGET_MAX : 1000);
          openTargetPopover("Stake target max", cur, (val)=>{
            localStorage.setItem(keyTarget("stake", a), String(val));
            toast("Stake target saved");
          });
        }, { passive:true });

        // place near percent text if exists, else append in host
        const pct = byId("stakePercent");
        (pct?.parentElement || host).appendChild(btn);
      }
    }

    // Reward
    const rewardBar = byId("rewardBar");
    if (rewardBar && !byId("rewardGearBtn")){
      const host = rewardBar.parentElement?.parentElement || rewardBar.parentElement;
      if (host){
        const btn = document.createElement("button");
        btn.id = "rewardGearBtn";
        btn.type = "button";
        btn.className = "patch-gear";
        btn.textContent = "⚙️";
        btn.style.marginLeft = "10px";
        btn.addEventListener("click", ()=>{
          const a = (typeof address !== "undefined") ? address : "";
          const cur = safe(localStorage.getItem(keyTarget("reward", a))) || 0;
          openTargetPopover("Reward target max", cur || 10, (val)=>{
            localStorage.setItem(keyTarget("reward", a), String(val));
            toast("Reward target saved");
          });
        }, { passive:true });

        const pct = byId("rewardPercent");
        (pct?.parentElement || host).appendChild(btn);
      }
    }
  }

  function applyTargetsToBars(){
    try{
      const a = (typeof address !== "undefined") ? address : "";
      const stakeMaxUser = safe(localStorage.getItem(keyTarget("stake", a))) || 0;
      const rewardMaxUser = safe(localStorage.getItem(keyTarget("reward", a))) || 0;

      // Stake bar override
      const stakeVal = safe((typeof displayed !== "undefined") ? displayed.stake : 0);
      const stakeMax = stakeMaxUser > 0 ? stakeMaxUser : safe((typeof STAKE_TARGET_MAX !== "undefined") ? STAKE_TARGET_MAX : 1000);
      if (stakeMax > 0){
        const pct = clamp((stakeVal / stakeMax) * 100, 0, 100);
        const bar = byId("stakeBar");
        const line = byId("stakeLine");
        if (bar){ bar.style.width = pct + "%"; bar.style.backgroundPosition = `${(100 - pct) * 0.6}% 0`; }
        if (line) line.style.left = pct + "%";
        const maxEl = byId("stakeMax"); if (maxEl) maxEl.textContent = String(stakeMax);
        const pctEl = byId("stakePercent"); if (pctEl) pctEl.textContent = pct.toFixed(1) + "%";
      }

      // Reward bar override
      const rewardVal = safe((typeof displayed !== "undefined") ? displayed.rewards : 0);
      const dynMax = Math.max(0.1, Math.ceil(rewardVal * 10) / 10);
      const rewardMax = rewardMaxUser > 0 ? rewardMaxUser : dynMax;
      if (rewardMax > 0){
        const pct = clamp((rewardVal / rewardMax) * 100, 0, 100);
        const bar = byId("rewardBar");
        const line = byId("rewardLine");
        if (bar){ bar.style.width = pct + "%"; bar.style.backgroundPosition = `${(100 - pct)}% 0`; }
        if (line) line.style.left = pct + "%";
        const maxEl = byId("rewardMax"); if (maxEl) maxEl.textContent = rewardMax.toFixed(1);
        const pctEl = byId("rewardPercent"); if (pctEl) pctEl.textContent = pct.toFixed(1) + "%";
      }
    }catch{}
  }

  /* ---------- 9) APR chart (points when APR changes) ---------- */
  ...
  function aprKey(addr){ return addr ? `inj_apr_series_v${APR_LOCAL_VER}_${addr}` : null; }
  ...
  function loadAprSeries(){
    try{
      ...
      if (!raw) return;
      const obj = JSON.parse(raw);
      if (!obj || obj.v !== APR_LOCAL_VER) return;
      aprT = Array.isArray(obj.t) ? obj.t : [];
      aprV = Array.isArray(obj.vv) ? obj.vv : [];
      if (aprV.length) aprLast = safe(aprV[aprV.length - 1]);
    }catch{}
  }
  ...
  function saveAprSeries(){
    try{
      ...
      const k = aprKey(a);
      if (!k) return;
      localStorage.setItem(k, JSON.stringify({ v: APR_LOCAL_VER, t: aprT, vv: aprV }));
    }catch{}
  }

  function ensureAprChartUI(){
    // Try to find a place near the APR value element
    const aprEl = byId("apr");
    if (!aprEl) return;
  ...
  function initAprChart(){
    try{
      const canvas = byId("aprChart");
      if (!canvas || !window.Chart) return;

      aprChart = new Chart(canvas, {
        type: "line",
        data: {
          labels: aprT.map(x => new Date(x).toLocaleString()),
          datasets: [{
            data: aprV,
            borderColor: "#3b82f6",
            backgroundColor: "rgba(59,130,246,.12)",
        ...
        options: {
          ...
          scales: {
            ...
            y: {
              position: "right",
              ticks: { color: axisTickColor?.() || "rgba(249,250,251,.60)", callback: (v)=> safe(v).toFixed(2) + "%" },
              grid: { color: axisGridColor?.() || "rgba(249,250,251,.10)" }
            }
          }
        }
      });

      // scale controls under APR chart
      ensureScaleControls("aprChart", "apr", ()=>aprChart);
    }catch{}
  }
  ...
  function drawAprChart(){
    try{
      ...
      if (!aprChart) return;
      aprChart.data.labels = aprT.map(x => new Date(x).toLocaleString());
      aprChart.data.datasets[0].data = aprV;
      aprChart.update("none");
      const sc = localStorage.getItem(getScaleKey("apr")) || "linear";
      setChartScale(aprChart, sc);
    }catch{}
  }
  ...
  function recordAprPoint(){
    try{
      const a = (typeof address !== "undefined") ? address : "";
      if (!a) return;
      const v = safe((typeof apr !== "undefined") ? apr : 0);
      if (!v) return;
      ...
      const diff = Math.abs(v - aprLast);
      if (diff < 0.01) return; // threshold
      ...
      // clamp
      if (aprT.length > 1200){
        aprT = aprT.slice(-1200);
        aprV = aprV.slice(-1200);
      }

      saveAprSeries();
      drawAprChart();

      // log event if your events exists
      if (typeof pushEvent === "function"){
        pushEvent({
          type:"apr",
          title:"APR changed",
          value:`${v.toFixed(2)}%`,
          status: (typeof hasInternet==="function" && hasInternet()) ? "ok" : "pending"
        });
      }
    }catch{}
  }

  /* ---------- 10) Advanced reset (menu/settings) per address ---------- */
  function ensureAdvancedResetUI(){
    const drawer = byId("drawer");
    if (!drawer) return;

    if (byId("advancedResetBtn")) return;

    // try to find drawer foot
    const foot = drawer.querySelector(".drawer-foot") || drawer;
    ...
    box.innerHTML = `
      <div style="font-weight:1000;letter-spacing:.02em;margin-bottom:8px;">Advanced</div>
      <button id="advancedResetBtn" type="button"
        style="width:100%;height:42px;border-radius:16px;border:1px solid rgba(255,255,255,.14);
        background:rgba(255,255,255,.06);font-weight:1000;cursor:pointer;">
        Reset charts for this wallet
      </button>
      <div style="margin-top:8px;opacity:.72;font-weight:850;font-size:.78rem;">
        Clears Stake/Reward/NetWorth/APR/Events history for the current address only.
      </div>
    `;
    foot.appendChild(box);

    box.querySelector("#advancedResetBtn")?.addEventListener("click", ()=>{
      const a = (typeof address !== "undefined") ? address : "";
      if (!a) return toast("No address selected");

      const ok = confirm("Reset ALL saved points for this wallet?\n\nThis will clear Stake/Reward/NetWorth/APR/Events history for the current address only.");
      if (!ok) return;

      try{
        // remove known keys
        const keys = [
          (typeof stakeStoreKey === "function") ? stakeStoreKey(a) : `inj_stake_series_v2_${a}`,
          (typeof wdStoreKey === "function") ? wdStoreKey(a) : `inj_reward_withdrawals_v2_${a}`,
          (typeof nwStoreKey === "function") ? nwStoreKey(a) : `inj_networth_v1_${a}`,
          (typeof evStoreKey === "function") ? evStoreKey(a) : `inj_events_v1_${a}`,
          aprKey(a),
          keyTarget("stake", a),
          keyTarget("reward", a)
        ].filter(Boolean);
        ...
        if (typeof nwTAll !== "undefined") nwTAll = [];
        if (typeof nwUsdAll !== "undefined") nwUsdAll = [];
        if (typeof nwInjAll !== "undefined") nwInjAll = [];
        if (typeof drawNW === "function") drawNW();

        aprT = []; aprV = []; aprLast = null;
        drawAprChart();

        if (typeof eventsAll !== "undefined") eventsAll = [];
        if (typeof renderEventRows === "function") renderEventRows();

        if (typeof cloudRenderCounts === "function") cloudRenderCounts();
  ...
  /* ---------- 11) Apply scale controls to existing charts ---------- */
  function ensureAllChartControls(){
    ensureScaleControls("netWorthChart", "networth", ()=> (typeof netWorthChart !== "undefined") ? netWorthChart : null);
    ensureScaleControls("stakeChart", "stake", ()=> (typeof stakeChart !== "undefined") ? stakeChart : null);
    ensureScaleControls("rewardChart", "reward", ()=> (typeof rewardChart !== "undefined") ? rewardChart : null);
    // Price uses its own control block that already includes LIN/LOG
  }

  /* ---------- 12) Light layout tuning: tighten right axis a bit (more space to plot) ---------- */
  function tightenRightAxis(){
    try{
      if (typeof netWorthChart !== "undefined" && netWorthChart){
        netWorthChart.options.layout = netWorthChart.options.layout || {};
        netWorthChart.options.layout.padding = netWorthChart.options.layout.padding || {};
        netWorthChart.options.layout.padding.right = 22; // was bigger
        if (netWorthChart.options.scales?.y?.ticks) netWorthChart.options.scales.y.ticks.padding = 4;
        netWorthChart.update("none");
      }
      if (typeof rewardChart !== "undefined" && rewardChart){
        if (rewardChart.options.scales?.y?.ticks){
          rewardChart.options.scales.y.ticks.mirror = true;
          rewardChart.options.scales.y.ticks.padding = 4;
        }
        rewardChart.update("none");
      }
      if (typeof stakeChart !== "undefined" && stakeChart){
        if (stakeChart.options.scales?.y?.ticks){
          stakeChart.options.scales.y.ticks.mirror = true;
          stakeChart.options.scales.y.ticks.padding = 4;
        }
        stakeChart.update("none");
      }
    }catch{}
  ...
  function bootPatch(){
    try{
      ensureCopyBtn();
      registerLastDotPlugin();
      ensurePriceTfUI();
      patchRewardFilterOptions();
      ensureRewardEstimatesUI();
      ensureGearButtons();
      ensureAllChartControls();
      ensureAprChartUI();
      loadAprSeries();
      initAprChart();
      drawAprChart();
      ensureAdvancedResetUI();
      tightenRightAxis();

      // apply saved scales
      try{
        if (typeof stakeChart !== "undefined" && stakeChart){
          const s = localStorage.getItem(getScaleKey("stake")) || "linear";
          setChartScale(stakeChart, s);
        }
        if (typeof rewardChart !== "undefined" && rewardChart){
          const s = localStorage.getItem(getScaleKey("reward")) || "linear";
          setChartScale(rewardChart, s);
        }
        if (typeof netWorthChart !== "undefined" && netWorthChart){
          const s = localStorage.getItem(getScaleKey("networth")) || "linear";
          setChartScale(netWorthChart, s);
        }
      }catch{}
    }catch{}
  }

  // Record APR point when APR changes (hook into animation loop indirectly)
  let _lastAprSeenTick = null;
  function patchLoop(){
    try{
      // bar targets
      applyTargetsToBars();
      ...
      // APR points
      const curApr = safe((typeof apr !== "undefined") ? apr : 0);
      if (curApr && (_lastAprSeenTick == null || Math.abs(curApr - _lastAprSeenTick) > 0.01)){
        _lastAprSeenTick = curApr;
        recordAprPoint();
      }

      // keep price series color coherent with selected timeframe
      applyPriceColorsFromSeries();

      // ensure validator refresh (cheap)
      // (no spam: only occasionally)
    }catch{}
