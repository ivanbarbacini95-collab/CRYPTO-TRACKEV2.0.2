...
function colorNumber(el,n,o,dec=4){
    if(n===o){el.innerHTML=n.toFixed(dec); return;}
...
async function fetchHistory(){
    ...
    price24hOpen=+d[0][1]; price24hLow=Math.min(...chartData); price24hHigh=Math.max(...chartData);
...
// ================= BAR UPDATE =================
function updateBar(bar,line,val,open,low,high,gradUp,gradDown){
    let leftPercent;
    if(val>=open){
        leftPercent=50;
        const widthPerc=((val-open)/(high-open))*50;
        bar.style.left="50%"; bar.style.width=widthPerc+"%"; bar.style.background=gradUp;
    } else {
        leftPercent=50-((open-val)/(open-low))*50;
        bar.style.left=leftPercent+"%"; bar.style.width=(50-leftPercent)+"%"; bar.style.background=gradDown;
    }
    line.style.left=((val-low)/(high-low))*100+"%";
}
