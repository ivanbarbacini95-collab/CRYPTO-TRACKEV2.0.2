...
/* ============ PATCH MIGLIORIE JAVASCRIPT ============ */

// 1. FUNZIONE PER SCALA LINEARE/LOGARITMICA
function setupScaleToggles() {
  // Net Worth Scale Toggle
  const nwScaleBtn = document.getElementById('nwScaleToggle');
  if (nwScaleBtn) {
    nwScaleBtn.addEventListener('click', function() {
      const isLinear = this.textContent === 'LIN';
      this.textContent = isLinear ? 'LOG' : 'LIN';
      this.classList.toggle('active', !isLinear);
      
      // Aggiorna grafico Net Worth
      if (netWorthChart) {
        netWorthChart.options.scales.y.type = isLinear ? 'logarithmic' : 'linear';
  ...
  // Reward Scale Toggle
  const rewardScaleBtns = document.querySelectorAll('.scale-btn');
  rewardScaleBtns.forEach(btn => {
    btn.addEventListener('click', function() {
      const scale = this.dataset.scale;
      document.querySelectorAll('.scale-btn').forEach(b => b.classList.remove('active'));
      this.classList.add('active');
      
      // Aggiorna grafico Rewards
      if (rewardChart) {
        rewardChart.options.scales.y.type = scale === 'log' ? 'logarithmic' : 'linear';
        rewardChart.update();
      }
    });
  });

  // Price Chart Scale Toggle
  const priceScaleBtn = document.getElementById('priceScaleToggle');
  if (priceScaleBtn) {
    priceScaleBtn.addEventListener('click', function() {
      const isLinear = this.textContent === 'LIN';
      this.textContent = isLinear ? 'LOG' : 'LIN';
      
      if (priceChart) {
        priceChart.options.scales.y.type = isLinear ? 'logarithmic' : 'linear';
        priceChart.update();
      }
    });
  }
...
// 2. AUTO-SCALING PER GRAFICI LIVE
function setupAutoScaling() {
  let lastUpdate = Date.now();
  const updateInterval = 1000; // 1 secondo
  
  function updateCharts() {
    const now = Date.now();
    if (now - lastUpdate < updateInterval) return;
    
    // Net Worth Chart auto-scaling
    if (netWorthChart && nwTf === 'live') {
      const data = netWorthChart.data.datasets[0].data;
      if (data.length > 0) {
        ...
        netWorthChart.options.scales.y.min = min - padding;
        netWorthChart.options.scales.y.max = max + padding;
        netWorthChart.update('none');
      }
    }
    
    // Reward Chart auto-scaling
    if (rewardChart) {
      const data = rewardChart.data.datasets[0].data;
      if (data.length > 0) {
        const min = Math.min(...data);
        const max = Math.max(...data);
        const padding = (max - min) * 0.1;
        
        rewardChart.options.scales.y.min = Math.max(0, min - padding);
        rewardChart.options.scales.y.max = max + padding;
        rewardChart.update('none');
      }
    }
...
// 3. PALLINI LAMPEGGIANTI PER GRAFICI LIVE
function setupBlinkingDots() {
  function createBlinkingDot(chartId, color = 'green') {
    const chart = document.getElementById(chartId);
    if (!chart) return null;
    
    const dot = document.createElement('div');
    dot.className = `blinking-dot dot-${color}`;
    dot.style.display = 'none';
    chart.parentElement.style.position = 'relative';
    chart.parentElement.appendChild(dot);
    
    return dot;
  }
  
  const dots = {
    networth: createBlinkingDot('netWorthChart', 'green'),
    price: createBlinkingDot('priceChart', 'green'),
    reward: createBlinkingDot('rewardChart', 'yellow'),
    stake: createBlinkingDot('stakeChart', 'yellow')
  };
  
  // Aggiorna posizione pallini
  function updateDotPosition(dot, chartInstance, index) {
    if (!dot || !chartInstance) return;
  ...
  // Animazione continua
  setInterval(() => {
    if (netWorthChart && dots.networth) {
      const dataLength = netWorthChart.data.datasets[0].data.length;
      updateDotPosition(dots.networth, netWorthChart, dataLength - 1);
    }
    
    if (priceChart && dots.price) {
      const dataLength = priceChart.data.datasets[0].data.length;
      updateDotPosition(dots.price, priceChart, dataLength - 1);
    }
    
    if (rewardChart && dots.reward) {
      const dataLength = rewardChart.data.datasets[0].data.length;
      updateDotPosition(dots.reward, rewardChart, dataLength - 1);
    }
    
    if (stakeChart && dots.stake) {
      const dataLength = stakeChart.data.datasets[0].data.length;
      updateDotPosition(dots.stake, stakeChart, dataLength - 1);
    }
  }, 100);
}
...
function setupCopyAddress() {
  ...
  const addressDisplay = document.getElementById('shortAddress');
  
  if (!copyBtn || !addressDisplay) return;
  
  copyBtn.addEventListener('click', async () => {
    const fullAddress = address || addressDisplay.textContent;
...
// 5. CONFIGURAZIONE RANGE PERSONALIZZATO
function setupRangeConfiguration() {
  const rangeConfigBtn = document.getElementById('rewardRangeConfig');
  const rangeModal = document.getElementById('rewardRangeModal');
  const applyBtn = document.getElementById('applyRewardRange');
  const rangeInput = document.getElementById('customRewardRange');
  
  if (!rangeConfigBtn || !rangeModal) return;
  
  rangeConfigBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    rangeModal.classList.toggle('show');
  });
  
  // Chiudi modal cliccando fuori
  document.addEventListener('click', (e) => {
    if (!rangeModal.contains(e.target) && e.target !== rangeConfigBtn) {
      rangeModal.classList.remove('show');
    }
  });
  
  if (applyBtn && rangeInput) {
    applyBtn.addEventListener('click', () => {
      const maxValue = parseFloat(rangeInput.value);
      if (!isNaN(maxValue) && maxValue > 0) {
        localStorage.setItem('rewardCustomMax', maxValue.toString());
        
        // Aggiorna grafico reward
        if (rewardChart) {
          rewardChart.options.scales.y.max = maxValue;
          rewardChart.update();
        }
...
// 6. FILTRI AVANZATI PER EVENTI
function setupEventFilters() {
  const filterSelect = document.getElementById('eventFilter');
  const eventRows = document.querySelectorAll('#eventsTbody tr');
  
  if (!filterSelect || !eventRows.length) return;
  
  filterSelect.addEventListener('change', function() {
    const filterValue = this.value;
    
    eventRows.forEach(row => {
      const eventType = row.querySelector('.ev-pill').textContent.toLowerCase();
...
// 7. TOOLS: INJECTIVE CONVERTER
function setupInjectiveConverter() {
  const eurInput = document.getElementById('eurAmount');
  const eurRate = document.getElementById('eurUsdRate');
  const usdAmount = document.getElementById('usdAmount');
  const injAmount = document.getElementById('injAmount');
  const currentPrice = document.getElementById('currentInjPrice');
  
  if (!eurInput) return;
  
  function updateConversion() {
    const eurValue = parseFloat(eurInput.value) || 0;
    const rate = parseFloat(eurRate.textContent) || 1.08;
    const injPrice = parseFloat(currentPrice.textContent.replace('$', '')) || targetPrice;
    
    const usdValue = eurValue * rate;
    const injValue = injPrice > 0 ? usdValue / injPrice : 0;
    
    usdAmount.textContent = `$${usdValue.toFixed(2)}`;
    injAmount.textContent = `${injValue.toFixed(4)} INJ`;
  }
  
  eurInput.addEventListener('input', updateConversion);
  
  // Aggiorna prezzo corrente periodicamente
  setInterval(() => {
    if (currentPrice && targetPrice > 0) {
      currentPrice.textContent = `$${targetPrice.toFixed(4)}`;
      updateConversion();
    }
  }, 5000);
}

// 8. TOOLS: MARKET CAP COMPARISON
function setupMarketCapComparison() {
  const compareSelect = document.getElementById('compareCoin');
  const formatSelect = document.getElementById('capFormat');
  const injCap = document.getElementById('injMarketCap');
  const compareCap = document.getElementById('compareMarketCap');
  const ratio = document.getElementById('marketCapRatio');
  
  if (!compareSelect) return;
  
  // Dati mock (nella realtà dovresti usare un'API)
  const marketCaps = {
    bitcoin: 1.2e12,    // 1.2 trilioni
    ethereum: 4.5e11,   // 450 miliardi
    solana: 8e10,       // 80 miliardi
    cardano: 1.5e10     // 15 miliardi
  };
  
  function updateComparison() {
    const selectedCoin = compareSelect.value;
    const format = formatSelect.value;
    const injMarketCap = 3.5e10; // 35 miliardi (mock)
    
    let compareValue = marketCaps[selectedCoin] || 0;
    let injValue = injMarketCap;
    ...
    const multiplier = formatMultiplier[format] || 1e-9;
    const suffix = format.charAt(0).toUpperCase();
    
    injCap.textContent = `$${(injValue * multiplier).toFixed(2)} ${suffix}`;
    compareCap.textContent = `$${(compareValue * multiplier).toFixed(2)} ${suffix}`;
    
    // Calcola ratio
    const ratioValue = compareValue > 0 ? (injValue / compareValue) * 100 : 0;
    ratio.textContent = `${ratioValue.toFixed(2)}%`;
  }
  
  compareSelect.addEventListener('change', updateComparison);
  formatSelect.addEventListener('change', updateComparison);
  
  // Aggiorna ogni minuto
  setInterval(updateComparison, 60000);
  updateComparison();
}
...
function setupDataReset() {
  ...
  const checkboxes = {
    staked: document.getElementById('resetStaked'),
    rewards: document.getElementById('resetRewards'),
    apr: document.getElementById('resetAPR'),
    all: document.getElementById('resetAll')
  };
  
  if (!resetBtn) return;
  
  resetBtn.addEventListener('click', () => {
    const resetStaked = checkboxes.staked.checked;
    const resetRewards = checkboxes.rewards.checked;
    const resetAPR = checkboxes.apr.checked;
    const resetAll = checkboxes.all.checked;
    
    if (!resetStaked && !resetRewards && !resetAPR && !resetAll) {
      alert('Seleziona almeno un tipo di dati da resettare.');
      return;
    }
    
    if (confirm('ATTENZIONE: Questa azione è irreversibile! Vuoi procedere?')) {
      // Reset dati selezionati
      if (resetAll || resetStaked) {
        localStorage.removeItem(`inj_stake_series_v${STAKE_LOCAL_VER}_${address}`);
        stakeLabels = [];
        stakeData = [];
        stakeMoves = [];
        stakeTypes = [];
      ...
      if (resetAll || resetRewards) {
        localStorage.removeItem(`inj_reward_withdrawals_v${REWARD_WD_LOCAL_VER}_${address}`);
        wdLabelsAll = [];
        wdValuesAll = [];
        wdTimesAll = [];
        rebuildWdView();
...
// 10. TIMEFRAME MULTIPLI PER GRAFICI
function setupMultipleTimeframes() {
  // Net Worth Timeframes
  const nwTimeframes = document.querySelectorAll('#nwTfSwitch .tf-btn');
  nwTimeframes.forEach(btn => {
    btn.addEventListener('click', function() {
      nwTf = this.dataset.tf;
      
      // Aggiorna grafico in base al timeframe
      const view = nwBuildView(nwTf);
      if (netWorthChart) {
        netWorthChart.data.labels = view.labels;
        netWorthChart.data.datasets[0].data = view.data;
      ...
      // Salva preferenza
      localStorage.setItem('netWorthTimeframe', nwTf);
    });
  });
  
  // Price Chart Timeframes
  const priceTimeframes = document.querySelectorAll('.tf-btn-above');
  priceTimeframes.forEach(btn => {
    btn.addEventListener('click', async function() {
      const timeframe = this.dataset.tf;
      
      // Rimuovi active da tutti
      priceTimeframes.forEach(b => b.classList.remove('active'));
      // Aggiungi active a quello cliccato
      this.classList.add('active');
      
      // Carica dati per il nuovo timeframe
      await loadChartData(timeframe);
    });
  });
}

// 11. ANIMAZIONE REFRESH MIGLIORATA
function setupRefreshAnimation() {
  const refreshBtn = document.getElementById('refreshMode');
  const liveIcon = document.getElementById('liveIcon');
  
  if (!refreshBtn || !liveIcon) return;
  
  refreshBtn.addEventListener('click', function() {
    if (liveMode) {
      // Modalità refresh - mostra animazione
      liveIcon.classList.add('refresh-animation');
      liveIcon.textContent = '⟳';
      
      // Simula caricamento
      setTimeout(() => {
        if (!liveMode) {
          liveIcon.classList.remove('refresh-animation');
        }
      }, 1000);
    }
  });
...
function setupEventCounter() {
  ...
  function updateEventCounter() {
    ...
    if (unreadEvents > 0) {
      eventBadge.textContent = unreadEvents;
  ...
  // Simula nuovi eventi (nella realtà dovresti ascoltare WebSocket)
  setInterval(() => {
    // Logica per nuovi eventi
    if (Math.random() > 0.8) { // 20% di probabilità
      unreadEvents++;
      updateEventCounter();
    }
  }, 30000);
  
  // Resetta counter quando si visita la pagina event
  document.addEventListener('click', (e) => {
    if (e.target.closest('[data-page="event"]')) {
      unreadEvents = 0;
      updateEventCounter();
    }
  });
...
function setupEventPagination() {
  ...
  function renderEventsPage() {
    ...
    const pageEvents = eventsAll.slice(startIndex, endIndex);
    
    // Renderizza solo gli eventi della pagina corrente
  ...
  function updatePagination() {
    const totalPages = Math.ceil(eventsAll.length / eventsPerPage);
  ...
  // Override renderEvents esistente
  const originalRenderEvents = window.renderEvents;
  window.renderEvents = function() {
    if (eventsAll.length > eventsPerPage) {
      renderEventsPage();
    } else {
...
function initializeImprovements() {
  ...
  function init() {
    ...
    setupEventFilters();
    setupInjectiveConverter();
    setupMarketCapComparison();
    setupDataReset();
    setupMultipleTimeframes();
    setupRefreshAnimation();
    setupEventCounter();
    setupEventPagination();
    
    // Aggiorna indirizzo breve
    updateShortAddress();
    
    console.log('Migliorie applicate con successo!');
  }
}

// 15. FUNZIONE AGGIORNA INDIRIZZO BREVE
function updateShortAddress() {
  const shortAddressEl = document.getElementById('shortAddress');
  if (shortAddressEl && address) {
    shortAddressEl.textContent = `${address.slice(0, 6)}...${address.slice(-4)}`;
  }
}

// 16. CARICA DATI GRAFICO PER TIMEFRAME
async function loadChartData(timeframe) {
  ...
  try {
    const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=${interval}&limit=${limit}`);
    const data = await response.json();
    
    if (priceChart && Array.isArray(data)) {
    ...
    console.error('Errore nel caricamento dati grafico:', error);
  }
}

// 17. GESTIONE ESPANSIONE CARD
...
// 18. AUTO-SCROLL PER PUNTI MULTIPLI
function setupAutoScroll() {
  const scrollableContainers = document.querySelectorAll('.chart-scrollable');
  
  scrollableContainers.forEach(container => {
    if (container.scrollWidth > container.clientWidth) {
      // Scorrimento automatico alla fine
      container.scrollLeft = container.scrollWidth;
    }
  });
}

// 19. AGGIORNAMENTO DINAMICO VALUTA
function setupCurrencyUpdates() {
  // Simula aggiornamento tasso EUR/USD
  setInterval(() => {
    const eurRate = document.getElementById('eurUsdRate');
    if (eurRate) {
      // Variazione casuale ±0.01
      const currentRate = parseFloat(eurRate.textContent) || 1.08;
      const change = (Math.random() - 0.5) * 0.02;
      eurRate.textContent = (currentRate + change).toFixed(4);
      
      // Aggiorna conversioni se necessario
      if (document.getElementById('eurAmount')?.value) {
        setupInjectiveConverter();
      }
    }
  }, 30000);
...
// 21. AVVIO TUTTE LE MIGLIORIE
document.addEventListener('DOMContentLoaded', function() {
  initializeImprovements();
  
  // Avvia anche queste funzioni aggiuntive
  setTimeout(() => {
    setupCardExpansion();
    setupAutoScroll();
    ...
    // Inizializza scale toggles
    const savedScale = localStorage.getItem('chartScale') || 'linear';
    ...
    // Inizializza timeframe preferito
    const savedTimeframe = localStorage.getItem('priceChartTimeframe') || '1d';
    document.querySelectorAll(`.tf-btn-above[data-tf="${savedTimeframe}"]`).forEach(btn => {
      btn.click();
    });
  }, 1000);
});
...
// 23. LOADING OVERLAY PER REFRESH
...
window.addEventListener('error', function(e) {
  ...
  toast.textContent = `Error: ${e.message || 'Unknown error'}`;
...
function monitorPerformance() {
  ...
  function checkFrame() {
    ...
    if (frameTime < 100) { // Ignora lunghi intervalli (come quando la tab è in background)
      performanceLog.push(frameTime);
    }
