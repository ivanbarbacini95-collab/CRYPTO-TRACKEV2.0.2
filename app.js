/* =========================================================
   ...
   Fix:
   - niente "pagine": dashboard sempre visibile
   - menu items solo nel drawer
   - search espande davvero e collassa dopo invio
   - dati: NON bloccati da ping, tornano a caricarsi
   - connessione: Offline/Loading/Online lenta e precisa
========================================================= */
...
/* ===================== CONNECTION UI ===================== */
/**
 * Qui non blocchiamo più i fetch.
 * Stato:
 * - Offline: navigator offline
 * - Loading: navigator online ma nessun successo recente / fallimenti
 * - Online: almeno 1 successo recente
 */
...
const SUCCESS_TO_ONLINE_MS = 8000; // se hai avuto un successo negli ultimi 8s => Online
const FAILS_TO_OFFLINE = 6;        // se fallisce tanto => rimane Loading (non Offline), Offline solo se navigator offline
const CONN_UI_TICK_MS = 1200;
...
function refreshConnUI(){
  ...
  if(lastSuccessTS && age <= SUCCESS_TO_ONLINE_MS){
    setConnState("online");
  } else {
...
setInterval(refreshConnUI, CONN_UI_TICK_MS);
window.addEventListener("online", () => { refreshConnUI(); if(getMode()==="live") restartMode(); }, { passive:true });
window.addEventListener("offline", refreshConnUI, { passive:true });
refreshConnUI();

function canFetch(){
  // permettiamo fetch se browser dice online
...
/* =========================================================
   UI: DRAWER + NAV (work in progress)
========================================================= */
...
drawerNav?.addEventListener("click",(e)=>{
  ...
  if(page && page !== "dashboard"){
    // solo qui appare "work in progress"
    alert("Work in progress 🙂");
    // torna dashboard come attiva
    [...drawerNav.querySelectorAll(".nav-item")].forEach(x=>{
      if(x.getAttribute("data-page")==="dashboard") x.classList.add("active");
...
/* =========================================================
   UI: SEARCH (expand + commit + collapse to lens)
========================================================= */
...
searchWrap?.addEventListener("click",(e)=>{
  // click sul wrap (non sul bottone) apre
  if(e.target === searchWrap || e.target === addressInput){
    if(!searchWrap.classList.contains("expanded")) expandSearch();
  }
});
...
function readMeta(prefix, addr){
  ...
  try{
    ...
    return { chunks: Math.max(0, parseInt(m?.chunks||0,10)), total: Math.max(0, parseInt(m?.total||0,10)) };
  }catch{ return { chunks:0, total:0 }; }
}
function writeMeta(prefix, addr, meta){
  const k=metaKey(prefix, addr);
  if(!k) return true;
  try{ localStorage.setItem(k, JSON.stringify(meta)); return true; }catch{ return false; }
}
...
function writeChunk(prefix, addr, idx, arr){
  const k=chunkKey(prefix, addr, idx);
  if(!k) return true;
  try{ localStorage.setItem(k, JSON.stringify(arr)); return true; }catch{ return false; }
}
...
function makeClippedPointLabelPlugin({ id, getLabel, font="800 11px Inter, sans-serif" }){
  return {
    ...
    afterDatasetsDraw(ch){
      ...
      ctx.font=font;
      ctx.fillStyle=(document.body.dataset.theme==="light") ? "rgba(17,24,39,0.88)" : "rgba(249,250,251,0.92)";
...
function updatePriceOverlayFromPinned(){
  ...
  if(pinnedIndex==null){ chartEl.textContent="--"; return; }
...
function setupPriceChartInteractions(){
  ...
  const getIndexFromEvent=(evt)=>{
    const pts=priceChart.getElementsAtEventForMode(evt,"index",{ intersect:false },false);
...
function scheduleTradeRetry(){ clearTradeRetry(); tradeRetryTimer=setTimeout(()=> startTradeWS(), 1400); }

function startTradeWS(){
  try{ wsTrade?.close(); }catch{}
  if(getMode()!=="live" || !canFetch()) return;
...
function scheduleKlineRetry(){ clearKlineRetry(); klineRetryTimer=setTimeout(()=> startKlineWS(), 1400); }

function applyKline(intervalKey,k){
  const t=safe(k.t), o=safe(k.o), h=safe(k.h), l=safe(k.l);
  if(o && h && l){
